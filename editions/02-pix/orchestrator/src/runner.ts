...
export function runCorrectnessTests(
  ...
  return runVitest(p, "", `results/${p.name}-correctness.json`);
}

/**
 * Roda uma prova isolada (um arquivo de teste). Usado pelo modo rodadas, que
 * aplica a mesma prova a todos os participantes antes de passar para a próxima.
 */
export function runRoundTests(
  p: Participant,
  testFile: string
): CorrectnessResult | null {
  const slug = testFile.replace(/\.test\.ts$/, "");
  return runVitest(p, testFile, `results/${p.name}-${slug}.json`);
}
...
function runVitest(
  ...
  const outputFile = path.join(ROOT, outputRelPath);

  try {
    execSync(
      `npx vitest run ${testFile} --reporter=json --outputFile="${outputFile}"`,
      {
        cwd: testsDir,
        encoding: "utf-8",
        stdio: "pipe",
...
export function runLoadTests(p: Participant, quiet = false): LoadResult | null {
